  ...
  return (
    <section className={compact ? "" : "kn-card p-5 border-[#e4d9c9] bg-[#fdf8f4] shadow-md overflow-hidden"}>
      {!compact && collapsible ? (
        ...
        <div className="mb-4 flex items-center justify-between border-b border-[#e4d9c9] pb-3">
          <div>
            <span className="kn-badge kn-badge-gold uppercase tracking-wider text-[11px]">Financial Escrow</span>
            <h2 className="text-xl font-extrabold text-[#1a1210] mt-0.5">
              {hi ? "पैसा पूल व एस्क्रो" : "Money Pool & Financial Escrow"}
            </h2>
          </div>
          {view.qtyLocked && (
            <span className="kn-badge kn-badge-success text-xs font-mono font-bold">
      ...
      <AnimatePresence initial={false}>
        {(isOpen || !collapsible || compact) && (
          <motion.div
            initial={collapsible ? { height: 0, opacity: 0 } : false}
            animate={{ height: "auto", opacity: 1 }}
            exit={collapsible ? { height: 0, opacity: 0 } : undefined}
            transition={{ duration: 0.25, ease: "easeInOut" }}
            className={collapsible && !compact ? "mt-4 pt-4 border-t border-[#e4d9c9] space-y-4" : "space-y-4"}
          >
